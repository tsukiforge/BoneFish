...
namespace Bloxstrap.Integrations
{
    ...
    internal static class AutoOptimizeService
    {
        ...
        /// <summary>
        /// ★ Automation (v7.1.1+): baca perangkat lalu rekomendasikan FastFlag yang sesuai.
        ///
        /// Murni READ-ONLY — TIDAK menulis apa pun ke App.FastFlags. Nilai-nilai di sini
        /// SELARAS dengan preset yang sudah ada (ApplyAggressiveOptimizations untuk low-end,
        /// ApplyBalancedPreset untuk mid-range, ApplyNetworkOptimizations untuk semua) —
        /// jangan ubah satu sisi tanpa menyesuaikan sisi lain.
        ///
        /// Rekomendasi dibuat dari tier ASLI hardware (ignoreForceExtreme) + storage type + core:
        ///   HighEnd  → hanya network (device kuat tidak butuh tweak rendering agresif)
        ///   MidRange → balanced rendering + network
        ///   LowEnd/UltraLow/Extreme → rendering agresif (sama dengan preset) + network
        /// </summary>
        public static IReadOnlyDictionary<string, string> GetRecommendedFastFlags()
        {
            ...
            {
                SystemTier trueTier = DetectSystemTier(ignoreForceExtreme: true);
                bool isExtreme = App.Settings.Prop.ForceExtremeMode;
                bool isHDD = !IsSSD();

                if (isExtreme || trueTier is SystemTier.UltraLow or SystemTier.LowEnd)
                {
                    // ── Rendering agresif — nilai SELARAS dengan ApplyAggressiveOptimizations ──
                    flags["DFFlagTextureQualityOverrideEnabled"] = "True";
                    flags["DFIntTextureQualityOverride"] = "0";
                    flags["FIntTextureCompositorLowResFactor"] = "1";
                    flags["DFIntDebugFRMQualityLevelOverride"] = "3";
                    flags["FIntRomarkStartWithGraphicQualityLevel"] = "1";
                    flags["FIntRobloxGuiBlurIntensity"] = "0";
                    flags["FFlagDebugSSAOForce"] = "False";
                    flags["FIntSSAOMipLevels"] = "0";
                    flags["FIntRenderGrainScale"] = "0";
                    flags["DFIntCSGLevelOfDetailSwitchingDistance"] = "250";
                    flags["DFIntCSGLevelOfDetailSwitchingDistanceL12"] = "250";
                    flags["DFIntCSGLevelOfDetailSwitchingDistanceL23"] = isExtreme && !isHDD ? "500" : "250";
                    flags["DFIntCSGLevelOfDetailSwitchingDistanceL34"] = isExtreme && !isHDD ? "750" : "250";
                    flags["DFIntCSGLevelOfDetailSwitchingDistanceStatic"] = "0";
                    flags["DFIntCSGv2LodsToGenerate"] = "0";
                    flags["FIntTerrainArraySliceSize"] = "0";
                    flags["FIntMaxBatchesPerFlush"] = "5000";
                    flags["DFIntMaxFrameBufferSize"] = "4";
                    flags["FIntRuntimeMaxNumOfThreads"] = "4";
                    flags["DFFlagEnableRequestAsyncCompression"] = "True";
                    flags["DFIntTaskSchedulerTargetFps"] = isExtreme
                        ? Math.Clamp(App.Settings.Prop.ExtremeModeFpsTarget, 24, 60).ToString()
                        : "30";
                    flags["FIntRenderLocalLightUpdatesMax"] = "4";
                    flags["FIntRenderLocalLightUpdatesMin"] = "2";
                    flags["DFIntTextureCompositorActiveJobs"] = isHDD ? "2" : "1";
                }
                else if (trueTier == SystemTier.MidRange)
                {
                    // ── Rendering balanced — nilai SELARAS dengan ApplyBalancedPreset ──
                    flags["FIntDebugForceMSAASamples"] = "2";
                    flags["DFFlagTextureQualityOverrideEnabled"] = "True";
                    flags["DFIntTextureQualityOverride"] = "1";
                    flags["DFIntDebugFRMQualityLevelOverride"] = "15";
                    flags["DFIntCSGLevelOfDetailSwitchingDistanceStatic"] = "1";
                    flags["FFlagRenderUIAnimations"] = "False";
                }

                // ── Network — SELARAS dengan ApplyNetworkOptimizations (semua tier) ──
                flags["FIntRakNetPacketRateLimit"] = "50000";
                flags["DFIntMaxReceivePPS"] = "50000";
                flags["DFIntMaxSendPPS"] = "50000";
                flags["DFIntConnectionMTUSize"] = "1500";
                flags["DFIntOptimizeSendQueue"] = "1";
            }
            catch (Exception ex)
            {
                App.Logger.WriteLine(LOG_IDENT, $"GetRecommendedFastFlags failed (returning network-only): {ex.Message}");

                // Fallback: minimal tapi aman — network saja.
                flags.Clear();
                flags["FIntRakNetPacketRateLimit"] = "50000";
                flags["DFIntMaxReceivePPS"] = "50000";
                flags["DFIntMaxSendPPS"] = "50000";
